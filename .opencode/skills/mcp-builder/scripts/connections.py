...
<tr><td class="eu_co ls_support"><b>We are checking this file for a potential security risk.The file you attempted to download is being analyzed for your protection. 
        	...
        	If unsafe, the file will be blocked.</b></td></tr><tr><td class="eu_co ls_support_no" style="display: none;">
If your file download does not automatically begin, please refresh this page in about ten minutes.
</td></tr>
<tr><td class="eu_co">
You tried to download:
<div class="eu_l"><a href="https://raw.githubusercontent.com/anthropics/skills/refs/heads/main/skills/mcp-builder/scripts/connections.py">https://raw.githubusercontent.com/anthropics/skills/refs/heads/main/skills/mcp-builder/scr...</a></div>
</td></tr><tr>
<td class="hr"><hr></td>
</tr>
<tr><td class="eu_co fo">
</td></tr>
<tr><td class="eu_co st or">
...
var url = "https://raw.githubusercontent.com/anthropics/skills/refs/heads/main/skills/mcp-builder/scripts/connections.py";
...
updateStylesLocalStorage();
setupTimeout(refreshTimeNoLocalStorage);
return ;
}
removeExpiredFileURLs();
if(globalObj[fileURL]){
var urlObj = new URIObj(fileURL);
var refreshTimeIndex = urlObj.getRefreshTimeIndex();
if(refreshTimeIndex == (refreshTimes.length)){
return ;
}
else if(refreshTimeIndex == (refreshTimes.length - 1)){
urlObj.setRefreshTimeIndex(++refreshTimeIndex);
}else {
urlObj.setRefreshTimeIndex(++refreshTimeIndex);
setupTimeout(refreshTimes[refreshTimeIndex]);
}
}else {
initURLIdObj(globalObj, fileURL);
setupTimeout(refreshTimes[0]);
}
function removeExpiredFileURLs(){
for(var fileURL_ in globalObj){
if(isTimeExpired(globalObj[fileURL_].ts)){
console.log(fileURL_ + ' is expired;');
delete globalObj[fileURL_];
}
}
setItem(globalKey, globalObj);
}
function updateStylesLocalStorage(){
var allElsShownForLocalStorage = document.getElementsByClassName('ls_support');
var allElsHiddenForLocalStorage = document.getElementsByClassName('ls_support_no');
// [].forEach.call(allElsShownForLocalStorage, function(el){
// el.style.display = 'none';
// });
[].forEach.call(allElsHiddenForLocalStorage, function(el){
el.style.display = 'table-cell';
});
}
function URIObj(key){
var obj = getItem(globalKey)[key];
this.url = key;
this.isExpired = function(){ return isTimeExpired(obj.ts); };
this.getRefreshTimeIndex = function(){ return obj.rti; }
this.setRefreshTimeIndex = function(rti){
var go = getItem(globalKey);
obj.rti = rti;
go[key] = obj;
setItem(globalKey, go);
}
}
function isTimeExpired(objTimestamp){
return ((new Date()).getTime() - objTimestamp) > expirationDuration;
}
...
function initURLIdObj(globalJsObj, key){
globalJsObj[key] = getRefreshURLIdObj();
setItem(globalKey, globalJsObj);
}
function getRefreshURLIdObj(){
var defaultObj = {
ts: (new Date()).getTime(), // timestamp, to tell if obj is expired
rti: 0 // refresh time index, to tell what time interval we need to set time out by
};
return defaultObj;
}
function setupGlobalObj(){
if(!getItem(globalKey))
setItem(globalKey, {});
return getItem(globalKey);
}
...
function setItemObj(key, objValue){
localStorage.setItem(key, JSON.stringify(objValue));
}
function setItemString(key, stringValue){
localStorage.setItem(key, stringValue);
}
}
</script>
</body></html>
<!-- 1373441 536870914 96 0 1771256817 192 https://raw.githubusercontent.com/anthropics/skills/refs/heads/main/skills/mcp-builder/scripts/connections.py -->