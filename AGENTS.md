...
- When consolidating duplicated Python modules behind alias wrappers, `sys.modules[__name__] = impl` alone is not enough for static analysis. Wrapper modules must also re-export public symbols for mypy, and explicitly bind any required private helpers (for example `_utcnow` or `_SAFE_BUILTINS`) that tests/routes import.
