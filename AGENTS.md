...
- Runtime-availability preflight for worker-manager-owned compute requests should be production-only. In tests, skip it with an explicit test env flag rather than registering fake live worker heartbeats into the shared settings DB; cross-thread heartbeat writes make settings/runtime tests flaky.
