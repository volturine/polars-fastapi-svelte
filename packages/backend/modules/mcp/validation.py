...
def _check_schema_node(schema: Any, path: str, issues: list[str]) -> None:
    ...
    for sub in (schema.get('not'), schema.get('if'), schema.get('then'), schema.get('else')):
    ...
    for prop, sub in schema.get('patternProperties', {}).items():
        _check_schema_node(sub, f'{path}.patternProperties.{prop}' if path else f'patternProperties.{prop}', issues)
    ap = schema.get('additionalProperties')
    if isinstance(ap, dict):
        _check_schema_node(ap, f'{path}.additionalProperties' if path else 'additionalProperties', issues)
