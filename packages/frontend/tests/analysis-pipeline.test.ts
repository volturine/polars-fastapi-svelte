...
/**
 * Create an analysis with pre-configured steps through the JSON import UI.
...
async function createPipelineAnalysis(
	...
	const analysisId = await createImportedAnalysis(
		request,
		name,
		{
			tabs: [
	...
	return { analysisId, viewStepId, tabId, resultId, dsId, steps: pipelineSteps };
}
