...
export async function createDatasource(
	...
	const response = await retry(
		() =>
			request.post(`${API_BASE}/datasource/upload`, {
				headers,
				multipart
			}),
		'createDatasource upload'
	);
	if (!response.ok()) {
...
async function retry<T>(action: () => Promise<T>, label: string): Promise<T> {
	let last: unknown;
	...
	throw new Error(`${label} failed after retries: ${String(last)}`);
}
