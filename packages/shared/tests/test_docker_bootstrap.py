...
def _wait_for_scheduled_build(schedule_id: str) -> tuple[str, str]:
    ...
    while time.time() < deadline:
        failure = _psql_value(
            "SELECT COALESCE(last_failure_at::text, '') "
            'FROM "default".schedules '
            f"WHERE id = '{schedule_id}' AND last_failure_at IS NOT NULL"
        )
        if failure:
    ...
    schedule_state = _psql_value(
        "SELECT concat(enabled, '|', COALESCE(next_run::text, ''), '|', COALESCE(lease_owner, '')) "
        'FROM "default".schedules '
        f"WHERE id = '{schedule_id}'"
    )
